...
def db_health():
    try:
        # Try to count documents in rooms collection as a simple DB check
...
def book_room():
    ...
    # Emit events for admin and hotel frontend
...
def delete_room(room_id):
    try:
        # Optionally, also delete any bookings for this room
...
def add_room():
    try:
        ...
        room = {
            ...
            "available": True,
            "bookedBy": None,
            "maintenance": False  # <-- ensure this is set
        }
...
def reset_rooms():
    try:
        ...
        bookings_collection.delete_many({})  # remove all bookings
        socketio.emit('resetRooms')  # broadcast to all clients
